...
class DashboardService
{
    ...
    public function headAdmin(Request $request)
    {
        ...
        $stats = [
            ...
            'pending_materials' => $this->dashboardRepository->materialRequests()->where('status', MaterialRequest::STATUS_PENDING)->count(),
            'open_issues' => $this->dashboardRepository->issueReports()->where('status', IssueReport::STATUS_OPEN)->count(),
        ...
        $materialsPager = $this->dashboardRepository->materialRequests()
        ...
        $issuesPager = $this->dashboardRepository->issueReports()
            ->with('foreman:id,fullname')
            ->latest()
        ...
        $recentPayrollsPager = $this->dashboardRepository->payrolls()
            ->latest()
    ...
    {
        $payrolls = $this->dashboardRepository->payrolls()->with('user')->latest()->take(20)->get();
        $totalPayable = (float) $this->dashboardRepository->payrolls()->whereIn('status', Payroll::payableStatuses())->sum('net');
        ...
        $projectPaymentsPager = $this->dashboardRepository->projects()
            ->orderBy('name')
    ...
    public function foreman(Request $request)
    {
        ...
        $attendances = $this->dashboardRepository->attendances()->where('foreman_id', $user->id)->latest()->take(20)->get();
        $accomplishments = $this->dashboardRepository->weeklyAccomplishments()->where('foreman_id', $user->id)->latest()->take(20)->get();
        $materialRequests = $this->dashboardRepository->materialRequests()->where('foreman_id', $user->id)->latest()->take(10)->get();
        $issueReports = $this->dashboardRepository->issueReports()->where('foreman_id', $user->id)->latest()->take(10)->get();
        $deliveries = $this->dashboardRepository->deliveries()
        ...
        $materialRequestsPager = $this->dashboardRepository->materialRequests()
            ->where('foreman_id', $user->id)
            ->latest()
        ...
        $issueReportsPager = $this->dashboardRepository->issueReports()
            ->where('foreman_id', $user->id)
            ->latest()
        ...
        $deliveriesPager = $this->dashboardRepository->deliveries()
            ->with('project:id,name')
            ->where('foreman_id', $user->id)
            ->latest()
        ...
        $weeklyAccomplishmentsByProjectQuery = $this->dashboardRepository->weeklyAccomplishments()
            ->with('project:id,name')
            ->where('foreman_id', $user->id);
            ...
            $weeklyAccomplishmentsByProjectPager->getCollection()->map(function (WeeklyAccomplishment $row) use ($user) {
                $lastSubmittedAt = $row->last_submitted_at
                    ? Carbon::parse((string) $row->last_submitted_at)
                        ->setTimezone(Attendance::PH_TIMEZONE)
                        ->toDateTimeString()
                    : null;
                ...
                if ($row->project_id !== null) {
                    $scopePhotoCountsByScope = $this->dashboardRepository->projectScopes()
                ...
                if ($latestWeekStart !== null) {
                    $latestScopeEntries = $this->dashboardRepository->weeklyAccomplishments()
                        ->where('foreman_id', $user->id)
        ...
        $progressPhotos = $this->dashboardRepository->progressPhotos()
            ->with('project:id,name')
            ->where('foreman_id', $user->id)
            ->when(
        ...
        $projectScopes = $this->dashboardRepository->projectScopes()
            ->when(
                $assignedProjectIds->isNotEmpty(),
        ...
        if ($foremanName !== '') {
            $phToday = Carbon::now(Attendance::PH_TIMEZONE)->toDateString();
            $selfLog = $this->dashboardRepository->attendances()
                ->with('project:id,name')
                ->where('foreman_id', $user->id)
                ->whereDate('date', $phToday)
                ->where('worker_role', Attendance::ROLE_FOREMAN)
    ...
    public function foremanSubmissions(Request $request)
    {
        ...
        $recentMaterialRequests = $this->dashboardRepository->materialRequests()
            ->with('project:id,name')
            ->where('foreman_id', $user->id)
            ->latest()
        ...
        $recentIssueReports = $this->dashboardRepository->issueReports()
            ->where('foreman_id', $user->id)
            ->latest()
        ...
        $recentDeliveries = $this->dashboardRepository->deliveries()
            ->with('project:id,name')
            ->where('foreman_id', $user->id)
            ->latest()
        ...
        $weeklyScopePhotos = $this->dashboardRepository->scopePhotos()
    ...
    {
        $payrollPayable = round((float) $this->dashboardRepository->payrolls()->whereIn('status', Payroll::payableStatuses())->sum('net'), 2);
        $payrollDeductionsTotal = round((float) $this->dashboardRepository->payrolls()->sum('deductions'), 2);
        $payrollPaidTotal = round((float) $this->dashboardRepository->payrolls()->where('status', Payroll::STATUS_PAID)->sum('net'), 2);
        $payrollGrossTotal = round((float) $this->dashboardRepository->payrolls()->sum('gross'), 2);

        $payrollCountsByStatus = $this->dashboardRepository->payrolls()
        ...
        $projects = $this->dashboardRepository->projects()->get(['contract_amount', 'total_client_payment', 'remaining_balance', 'status']);
    ...
    private function companyFinancialSummary(float $totalContractValue): array
    {
        ...
        $this->dashboardRepository->expenses()
            ->get(['category', 'amount'])
            ->each(function (Expense $expense) use (&$materials, &$laborFromExpenses, &$subcontractors, &$miscellaneous, &$equipment): void {
                ...
                if ($category === '' || $category === Str::lower(Expense::CATEGORY_UNCATEGORIZED_KEY)) {
        ...
        $payrollLabor = round((float) $this->dashboardRepository->payrolls()->sum('net'), 2);
    ...
    {
        $assigned = $this->dashboardRepository->projectAssignments()
        ...
        $fallback = $this->dashboardRepository->projects()
    ...
    private function includeTransferredProjectIds(Collection $projectIds): Collection
    {
        ...
        $transferredIds = $this->dashboardRepository->projects()
    ...
    private function excludeDesignPhaseIds(Collection $projectIds): Collection
    {
        ...
        $designIds = $this->dashboardRepository->projects()
            ->whereIn('id', $projectIds->all())
            ->whereRaw('LOWER(TRIM(phase)) = ?', [Str::lower(ProjectFlow::normalizePhase('Design'))])
            ->pluck('id')
            ->map(fn ($projectId) => (int) $projectId)
            ->values();
    ...
    private function foremanAssignedProjectsPayload(User $user, Collection $assignedProjectIds): array
    {
        ...
        return $this->dashboardRepository->projects()
    ...
    {
        $existing = $this->dashboardRepository->progressSubmitTokens()
        ...
        return $this->dashboardRepository->createProgressSubmitToken([
