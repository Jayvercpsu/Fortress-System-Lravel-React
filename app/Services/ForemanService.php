...
class ForemanService
{
    ...
    public function attendanceIndex(Request $request)
    {
        ...
        $query = $this->foremanProgressRepository->attendances()
        ...
        $attendances = collect($paginator->items())->map(function (Attendance $attendance) use ($foremanName, $phToday) {
            $isForemanSelfLog = $attendance->worker_role === Attendance::ROLE_FOREMAN
                && $foremanName !== ''
        ...
        if ($foremanId && $foremanName !== '') {
            $selfLog = $this->foremanProgressRepository->attendances()
                ->where('foreman_id', $foremanId)
        ...
        $stats = [
            'today_logs' => $this->foremanProgressRepository->attendances()->where('foreman_id', $foremanId)->whereDate('date', $phToday)->count(),
            'this_week_hours' => round((float) $this->foremanProgressRepository->attendances()->where('foreman_id', $foremanId)->whereBetween('date', [$phWeekStart, $phWeekEnd])->sum('hours'), 1),
            'total_logs' => $this->foremanProgressRepository->attendances()->where('foreman_id', $foremanId)->count(),
        ];
    ...
    public function timeInAttendance(Request $request)
    {
        ...
        if ($foremanName === '') {
            return $this->foremanActionRedirect($request)
                ->with('error', __('messages.foreman.full_name_required'));
        }

        $manilaNow = Carbon::now(Attendance::PH_TIMEZONE);
        $phToday = $manilaNow->toDateString();

        $existingToday = $this->foremanProgressRepository->attendances()
        ...
        if ($existingToday) {
            return $this->foremanActionRedirect($request)
                ->with('error', __('messages.foreman.time_in_already_recorded'));
        }

        $this->foremanProgressRepository->attendances()->create([
            'foreman_id' => $foreman->id,
            'project_id' => $validated['project_id'],
            'worker_name' => $foremanName,
            'worker_role' => Attendance::ROLE_FOREMAN,
            'date' => $phToday,
        ...
        return $this->foremanActionRedirect($request)
            ->with('success', __('messages.foreman.time_in_recorded'));
    }
    ...
    public function timeOutAttendance(Request $request)
    {
        ...
        if (!$selfLog) {
            return $this->foremanActionRedirect($request)
                ->with('error', __('messages.foreman.time_in_not_found'));
        }

        if (!empty($selfLog->time_out)) {
            return $this->foremanActionRedirect($request)
                ->with('error', __('messages.foreman.time_out_already_recorded'));
        }
        ...
        return $this->foremanActionRedirect($request)
            ->with('success', __('messages.foreman.time_out_recorded'));
    }
    ...
    public function updateAttendance(Request $request, Attendance $attendance)
    {
        ...
        if ((string) optional($attendance->date)?->toDateString() !== Carbon::now(Attendance::PH_TIMEZONE)->toDateString()) {
            return redirect()
                ...
                ->with('error', __('messages.foreman.attendance_today_only'));
        }
        ...
        return redirect()
            ->route('foreman.attendance.index', $this->attendanceTableQueryParams($request))
            ->with('success', __('messages.foreman.attendance_log_updated'));
    }
    ...
    public function storeAttendance(Request $request)
    {
        ...
        return redirect()
            ...
            ->with('success', __('messages.foreman.attendance_submitted'));
    }
    ...
    public function submitAll(Request $request)
    {
        ...
        $request->validate([
            ...
            'status'                             => ['nullable', Rule::in(DeliveryConfirmation::statusOptions())],
        ]);
        ...
        if (!empty($request->item_delivered) && !empty($request->delivery_date)) {
            $this->foremanProgressRepository->deliveries()->create([
                ...
                'status'         => $request->status ?? DeliveryConfirmation::STATUS_RECEIVED,
            ]);
        }

        return back()->with('success', __('messages.foreman.all_entries_submitted'));
    }
    ...
    public function storeProgressPhoto(Request $request)
    {
        ...
        if ($request->filled('scope_id')) {
            $scope = $this->foremanProgressRepository->projectScopes()->findOrFail((int) $request->scope_id);

            if ($projectId !== null && (int) $scope->project_id !== $projectId) {
                return back()
                    ->withErrors(['scope_id' => __('messages.foreman.scope_project_mismatch')])
                    ->withInput();
            }
        ...
        return back()->with('success', __('messages.foreman.photo_uploaded'));
    }
    ...
    private function createAttendanceEntries(array $entries, int $foremanId): void
    {
        foreach ($entries as $entry) {
            $this->foremanProgressRepository->attendances()->create([
                ...
                'date' => !empty($entry['date']) ? $entry['date'] : Carbon::now(Attendance::PH_TIMEZONE)->toDateString(),
    ...
    private function normalizeAttendanceFilterDate(mixed $rawDate, string $fallbackDate): string
    {
        ...
        try {
            return Carbon::createFromFormat('Y-m-d', $value, Attendance::PH_TIMEZONE)->toDateString();
        } catch (\Throwable $e) {
            return $fallbackDate;
        }
    }
    ...
    {
        $assigned = $this->foremanProgressRepository->projectAssignments()
        ...
        $fallback = $this->foremanProgressRepository->projects()
    ...
    private function includeTransferredProjectIds(Collection $projectIds): Collection
    {
        ...
        $transferredIds = $this->foremanProgressRepository->projects()
    ...
    private function excludeDesignPhaseIds(Collection $projectIds): Collection
    {
        ...
        $designIds = $this->foremanProgressRepository->projects()
    ...
    {
        return ProjectScope::statusFromProgress($progress);
    }
